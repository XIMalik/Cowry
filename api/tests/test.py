...
class LibraryAPITest(TestCase):
    ...
    def test_fetch_borrowed_books(self):
        url = reverse("fetch_borrowed_books")
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    
    def test_fetch_unavailable_books(self):
        url = reverse("fetch_unavailable_books")
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    
    def test_fetch_available_books(self):
        url = reverse("fetch_available_books")
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
